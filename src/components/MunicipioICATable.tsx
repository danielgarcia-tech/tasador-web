...
interface MunicipioICA {
  id?: string;
  pj: string;
  ica_aplicable: string;
}

interface MunicipioICATableProps {
  isAdmin: boolean;
}

const MunicipioICATable: React.FC<MunicipioICATableProps> = ({ isAdmin }) => {
  const [municipios, setMunicipios] = useState<MunicipioICA[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState<Partial<MunicipioICA>>({});
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [createForm, setCreateForm] = useState<Omit<MunicipioICA, 'id'>>({
    pj: '',
    ica_aplicable: '',
  });
  ...
  const handleEdit = (municipio: MunicipioICA) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Gestión de Municipios (PJ - ICA)</h3>
        {isAdmin && (
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg">
        <div>
          <label className="block text-sm font-medium mb-1">Filtrar por Partido Judicial</label>
          <input
            type="text"
            value={filterPJ}
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Filtrar por ICA Aplicable</label>
          <input
            type="text"
            value={filterICA}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value={createForm.pj}
                onChange={(e) => setCreateForm(prev => ({ ...prev, pj: e.target.value.toUpperCase() }))}
                className="w-full border rounded px-3 py-2"
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-200 rounded-lg">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Partido Judicial (PJ)</th>
                ...
                <td className="px-4 py-3 text-sm font-mono">
                  {editingId === municipio.id ? (
                    <input
                      type="text"
                      value={editForm.pj || ''}
                      onChange={(e) => setEditForm(prev => ({ ...prev, pj: e.target.value.toUpperCase() }))}
...
export default MunicipioICATable;